...
common::Status InferenceSession::TransformGraph(onnxruntime::Graph& graph,
  ...
  for (int i = static_cast<int>(TransformerLevel::Level1); i < static_cast<int>(TransformerLevel::MaxTransformerLevel);
       i++) {
...
common::Status InferenceSession::InitializeSubgraphSessions(Graph& graph, SessionState& session_state) {
  for (auto& node : graph.Nodes()) {
    ...
    for (const auto& entry : node.GetAttributeNameToMutableSubgraphMap()) {
      ...
      ORT_RETURN_IF_ERROR(
          initializer.CreatePlan(&node, &implicit_inputs, session_options_.enable_sequential_execution));
...
common::Status InferenceSession::Initialize() {
  ...
  try {
    ...
    ORT_RETURN_IF_ERROR(TransformGraph(graph, graph_transformation_mgr_, execution_providers_, kernel_registry_manager_,
                                       insert_cast_transformer_, session_state_));
